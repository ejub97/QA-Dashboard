...
mongo_url = os.getenv("MONGODB_URI", "mongodb+srv://ejubhasanovic:Test%40123@cluster0.d11pnra.mongodb.net/qa_dashboard?retryWrites=true&w=majority")
...
# serve attachment files from uploads directory
...
class TestCase(BaseModel):
    ...
    # attachments are stored as list of URLs
    steps_attachments: List[str] = []
...
class TestCaseCreate(BaseModel):
    ...
    expected_result: str
    actual_result: Optional[str] = ""
    steps_attachments: Optional[List[str]] = []
    expected_attachments: Optional[List[str]] = []
    actual_attachments: Optional[List[str]] = []
    assigned_to: Optional[str] = ""
...
class TestCaseUpdate(BaseModel):
    ...
    # attachments may be updated separate via endpoint
    steps_attachments: Optional[List[str]] = None
    expected_attachments: Optional[List[str]] = None
    actual_attachments: Optional[List[str]] = None
...
async def get_project_tabs(project_id: str, current_user: dict = Depends(get_current_user)):
    ...
    tabs = list(set([tc.get('tab', 'Sheet1') for tc in test_cases]))
    if not tabs:
        tabs = ["Sheet1"]
...
@api_router.post("/test-cases/{test_case_id}/attachments")
async def upload_test_case_attachments(
    ...
    """Upload one or more files for a particular field on a test case.
    Supported fields are 'steps', 'expected', and 'actual'.
    Files are stored under uploads/<tc_id>/<field>/ and the URLs are
    appended to the corresponding attachment list in the document.
    """
    # validate field name
    if field not in ("steps", "expected", "actual"):
        raise HTTPException(status_code=400, detail="Invalid attachment field")

    tc = await db.test_cases.find_one({"id": test_case_id})
    if not tc:
        raise HTTPException(status_code=404, detail="Test case not found")
    # permission check based on project
    await check_project_permission(tc["project_id"], current_user, "editor")

    base_dir = uploads_path / test_case_id / field
    ...
    for upload in files:
        ...
        with open(target, "wb") as f:
            ...
            f.write(content)
        # create url relative to mounted path
        urls.append(f"/uploads/{test_case_id}/{field}/{filename}")

    # push urls into document
    key = f"{field}_attachments"
    await db.test_cases.update_one({"id": test_case_id}, {"$push": {key: {"$each": urls}}})
...
Missing columns in your file: """ + ', '.join(missing_columns)
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                tab = str(row.get('Tab', row.get('tab', 'Sheet1'))).strip()
...
async def export_docx(project_id: str, current_user: dict = Depends(get_current_user)):
    ...
    test_cases = await db.test_cases.find({"project_id": project_id, "is_template": False}, {"_id": 0}).to_list(1000)
    test_cases.sort(key=lambda x: (x.get('tab', 'Sheet1'), x.get('created_at', '')))
    ...
    for idx, tc in enumerate(test_cases, start=1):
        ...
        row_cells[1].text = tc.get('tab', 'Sheet1')
