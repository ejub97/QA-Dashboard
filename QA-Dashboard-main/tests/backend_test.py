...
class QADashboardImportTester:
    ...
    def test_create_test_case(self):
        ...
        test_case_data = {
            ...
            "expected_result": "Test case should be created ssuccessfully",
            "actual_result": ""
        }
        success, response = self.run_test(
        ...
        if success and 'id' in response:
            ...
            # upload a dummy attachment to the steps field
            tmp = tempfile.NamedTemporaryFile(mode='wb', suffix='.txt', delete=False)
            tmp.write(b'Attachment for testing')
            tmp.flush()
            tmp.close()
            upload_url = f"{self.api_url}/test-cases/{self.test_case_id}/attachments?field=steps"
            with open(tmp.name, 'rb') as f:
                files = {'files': ('note.txt', f, 'text/plain')}
                headers = {'Authorization': f'Bearer {self.access_token}'}
                resp = requests.post(upload_url, headers=headers, files=files)
            if resp.status_code == 200:
                print(f"   ✅ Attachment uploaded successfully: {resp.json()}")
            else:
                print(f"   ❌ Attachment upload failed: {resp.status_code} {resp.text}")
            # verify that GET returns attachment url
            success2, check = self.run_test(
            ...
            if success2:
                ...
                for tc in check:
                    if tc.get('id') == self.test_case_id:
                        if tc.get('steps_attachments'):
                            found = True
                            print(f"   ✅ Retrieved attachments in test case record")
                if not found:
                    print("   ❌ Attachment URL missing from stored test case")
            return True
        return False
