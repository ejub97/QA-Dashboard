...
const BASE_URL = BACKEND_URL; // used for static asset links (uploads)
...
const TestCaseList = ({ project, userRole }) => {
  ...
  const [tabs, setTabs] = useState(['Sheet1']);
  ...
  const handleCreateTestCase = async (data) => {
    try {
      // data may include attachments property
      const { attachments, ...fields } = data;
      const response = await axios.post(`${API}/test-cases`, { ...fields, project_id: project.id });
      const newTc = response.data;
      setTestCases([...testCases, newTc]);

      // if there are files to upload, send them to new endpoint
      if (attachments) {
        const upload = async (fieldName, filesArray) => {
          if (!filesArray || filesArray.length === 0) return;
          const form = new FormData();
          filesArray.forEach(f => form.append('files', f));
          try {
            await axios.post(
              `${API}/test-cases/${newTc.id}/attachments?field=${fieldName}`,
              form,
              { headers: { 'Content-Type': 'multipart/form-data' } }
            );
          } catch (err) {
            console.error('Attachment upload failed', fieldName, err);
          }
        };

        await upload('steps', attachments.steps);
        await upload('expected', attachments.expected);
        await upload('actual', attachments.actual);

        // reload newly created test case to include attachment urls
        const updated = await axios.get(`${API}/test-cases`, { params: { project_id: project.id, tab: data.tab } });
        // refresh list
        setTestCases(updated.data);
      }
  ...
  const handleUpdateTestCase = async (data) => {
    try {
      const { attachments, ...fields } = data;
      const response = await axios.put(`${API}/test-cases/${editingTestCase.id}`, fields);
      let updatedTc = response.data;
      ...
      if (attachments) {
        const upload = async (fieldName, filesArray) => {
          ...
          try {
            await axios.post(
              `${API}/test-cases/${editingTestCase.id}/attachments?field=${fieldName}`,
              form,
              { headers: { 'Content-Type': 'multipart/form-data' } }
            );
          } catch (err) {
        ...
        const fresh = await axios.get(`${API}/test-cases`, { params: { project_id: project.id, tab: editingTestCase.tab } });
        const match = fresh.data.find(tc => tc.id === editingTestCase.id);
        if (match) updatedTc = match;
      }

      setTestCases(testCases.map((tc) => (tc.id === editingTestCase.id ? updatedTc : tc)));
                            ...
                            {expandedCases[tc.id] && (
                              <>
                                <div className="space-y-3 text-sm mb-3">
                                  ...
                                  {/* attachments preview */}
                                  {(tc.steps_attachments?.length || tc.expected_attachments?.length || tc.actual_attachments?.length) && (
                                    <div>
                                      <span className="font-medium text-gray-700">Attachments:</span>
                                      <div className="text-gray-600 mt-1 space-y-1">
                                        {[
                                          ...(tc.steps_attachments || []),
                                          ...(tc.expected_attachments || []),
                                          ...(tc.actual_attachments || [])
                                        ].map((url, idx) => {
                                          const normalized = url.startsWith('/') ? url : `/${url}`;
                                          const link = `${BASE_URL}${normalized}`;
                                          return (
                                            <div key={idx}>
                                              <a
                                                href={link}
                                                ...
                                                data-testid={`attachment-${tc.id}-${idx}`}
                                              >
