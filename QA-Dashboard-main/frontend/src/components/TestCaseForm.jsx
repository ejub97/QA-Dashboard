...
const TestCaseForm = ({ testCase, onSubmit, onCancel, currentTab }) => {
  ...
  // file attachments for the three rich text areas
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // include attachments object when invoking parent callback
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <div className="mt-2">
          <Label htmlFor="steps_files" className="text-sm">Attach files</Label>
          <Input
            id="steps_files"
            type="file"
            multiple
            accept=".png,.jpg,.jpeg,.gif,.mp4,.mov,.pdf"
            onChange={(e) => setAttachments(prev => ({ ...prev, steps: Array.from(e.target.files) }))}
            className="input-focus"
            data-testid="testcase-steps-files-input"
          />
          {attachments.steps.length > 0 && (
            <p className="text-xs text-gray-600 mt-1">
              Selected: {attachments.steps.map(f => f.name).join(', ')}
            </p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        />
        <div className="mt-2">
          <Label htmlFor="expected_files" className="text-sm">Attach files</Label>
          <Input
            id="expected_files"
            type="file"
            multiple
            accept=".png,.jpg,.jpeg,.gif,.mp4,.mov,.pdf"
            onChange={(e) => setAttachments(prev => ({ ...prev, expected: Array.from(e.target.files) }))}
            className="input-focus"
            data-testid="testcase-expected-files-input"
          />
          {attachments.expected.length > 0 && (
            <p className="text-xs text-gray-600 mt-1">
              Selected: {attachments.expected.map(f => f.name).join(', ')}
            </p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        />
        <div className="mt-2">
          <Label htmlFor="actual_files" className="text-sm">Attach files</Label>
          <Input
            id="actual_files"
            type="file"
            multiple
            accept=".png,.jpg,.jpeg,.gif,.mp4,.mov,.pdf"
            onChange={(e) => setAttachments(prev => ({ ...prev, actual: Array.from(e.target.files) }))}
            className="input-focus"
            data-testid="testcase-actual-files-input"
          />
          {attachments.actual.length > 0 && (
            <p className="text-xs text-gray-600 mt-1">
              Selected: {attachments.actual.map(f => f.name).join(', ')}
            </p>
          )}
        </div>
      </div>
